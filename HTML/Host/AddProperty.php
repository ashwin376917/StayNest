...
if (!isset($_SESSION['host_id'])) {
    ...
    echo "<script>alert('Please log in as a host to add a property.'); window.location.href = '../Guest/login.php';</script>";
// Adjusted path
    exit();
}
...
// Get state from new combobox
    $district = trim($_POST['district']);
// Get district from new combobox
    $category = $_POST['category'];
// Maps to 'categories'
    $description = $_POST['property_description']; // Maps to 'description'
    $price = floatval($_POST['price']);
// Maps to 'price_per_night', converted to float
...
// This is correct for retrieving from session.
...
return $newId;
    ...
    $homestay_status = 'pending';
// Maps to 'homestay_status' (or approval_status)
    $total_click = 0; // Maps to 'total_click'.
    // $isBan = 0; // Maps to 'isBan' (0 for not banned, 1 for banned) - REMOVED

    // Directory for image uploads
    $uploadDir = 'uploads/';
// Path relative to current script (CreateNest.php)
...
$picture2 = '';
    $picture3 = '';
    $uploadedFilePaths = [];
    $requiredImagesCount = 3;
// Number of images required

    // Process each image file individually based on their input names
    $imageInputNames = ['image1', 'image2', 'image3'];
// These match the 'name' attributes in the HTML
...
$homestay_id . "_img" . ($index + 1) . "_" . time() . "_" . $fileName;
...
$sql = "INSERT INTO homestay
             (homestay_id, host_id, title, description, district, state, price_per_night, amenities, categories, homestay_status, picture1, picture2, picture3, total_click, max_guests)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
// 
...
// Default values for other fields based on the database schema
$homestay_status_int = 0;
// Assuming 'pending' translates to 0 for TINYINT(1) 
$total_click = 0; // Maps to 'total_click'. 
// $isBan = 0; // Maps to 'isBan' (0 for not banned, 1 for banned) - REMOVED

// Corrected bind_param call
$stmt->bind_param(
    "ssssssdssisssii", // Corrected: 9th 'i' for category changed to 's'. 10th 's' for homestay_status changed to 'i'. Removed one 'i' for isBan.
...
<div class="property-container">
  ...
  <div class="image-gallery">
    <div id="imageBox0" class="image-box main-image shadow" data-index="0">
      ...
      <input type="file" name="image1" id="uploadImage0" accept="image/*" style="display: none;"
required />
    ...
    <div class="side-images">
      <div id="imageBox1" class="image-box side-image shadow" data-index="1">
        ...
        <input type="file" name="image2" id="uploadImage1" accept="image/*" style="display: none;"
disabled required />
      ...
      <div id="imageBox2" class="image-box side-image shadow" data-index="2">
        ...
        <input type="file" name="image3" id="uploadImage2" accept="image/*" style="display: none;"
disabled required />
  ...
  <div class="amenities-box shadow">
    ...
    <div class="amenities-list">
      <?php
      $amenities = [
        "Wifi", "Parking", "Kitchen", "Pool", "SmartTV",
        "PersonalWorkspace", "Washer", "HairDryer", "Dryer", "Aircond"
      ];
...
htmlspecialchars($a) . '</span>
  ...
  <div class="description-box shadow">
    <img id="descriptionBG" 
src="../../assets/accommodation-deluxeRoom.jpg" alt="BG" class="description-img" />
...
// All clickable image container divs
  const fileInputs = document.querySelectorAll('.image-gallery input[type="file"]');
// All hidden file inputs
  const previewImages = document.querySelectorAll('.image-gallery img.preview-image');
// All img elements for previews
  const descriptionBG = document.getElementById('descriptionBG');
// The description background image

  let currentUploadIndex = 0;
// Tracks which image slot is next to be filled
...
//Set the source of the preview image
            ...
            // Update box classes
            box.classList.remove('active-upload'); // Remove 
//active state from current box
...
// Enable clicks on the next box
              nextFileInput.disabled = false;
// Enable the next hidden file input
            } else {
...
"Pahang": ["Kuantan", "Bentong", "Bera", "Cameron Highlands", "Jerantut", "Kuala Lipis", "Maran", "Pekan", "Raub", "Rompin", "Temerloh"],
    ...
    "Sarawak": ["Kuching", "Bintulu", "Kapit", "Limbang", "Miri", "Mukah", "Samarahan", "Sibu", "Sri Aman", "Betong", "Sarikei", 
"Serian"],
